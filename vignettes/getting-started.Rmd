...
forestplot(estimate = 0.75, lower = 0.5, upper = 1.1)
```
...
forestplot(
  ...
  labs(title = "Treatment Effect", subtitle = "Odds Ratio with 95% CI")
```
...
plot_obj <- plot(
  ...
  layers = list(
    list(type = "boxplot"),
    list(
      type = "generic", geom = "ggplot2::geom_jitter",
      alpha = 0.6, width = 0.2
    )
  )
)
...
clinical_data <- tibble(
  ...
  response = sample(c("No Response", "Response"), 200,
    replace = TRUE,
    prob = c(0.6, 0.4)
  ),
...
styled_plot <- plot(
  data = mtcars,
  mapping = list(x = "factor(cyl)", y = "mpg"),
  layers = list(
    list(type = "boxplot"),
    list(type = "dotplot")
  )
...
Welcome to the nightowl community! 🦉✨
