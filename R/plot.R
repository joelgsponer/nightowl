  ...
  public = list(
    ...
    transform_data = function() {
      ...
      if (!is.null(transform)) {
        ...
        transform <- transform[pop(names(transform), "data")]
        purrr::iwalk(transform, function(.f, .var) {
          if (is.character(.f)) .f <- getfun(.f)
          .var <- mapping[[.var]]
          if (!is.null(.var)) {
            .data <<- .data %>%
        ...
        if (!is.null(transform_data)) {
          if (is.character(transform_data)) .f <- getfun(transform_data)
    ...
    prepare_facets = function() {
      ...
      self$mapping <- mapping[pop(names(mapping), c("facet_row", "facet_col"))]
      self$facets <- facets
    },
    # ggplot ---
    plot = NULL,
